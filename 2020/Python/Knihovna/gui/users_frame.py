...
class UsersFrame(tkinter.Frame):
    ...
    def btn_add_clicked(self):
        ...
        user = User(init_list, DatabaseDriver.instance)
        DatabaseDriver.instance.save_user(user)

        self.refresh_list()

    def list_box_on_selected(self, evt):
        selected_user = self.get_list_selected()
        if selected_user is None:
            return
        self.txt_name_var.set(selected_user.name)
        self.txt_surname_var.set(selected_user.surname)
        self.txt_birthday_var.set(selected_user.birthdate)

    def btn_delete_clicked(self):
        user = self.get_list_selected()
        DatabaseDriver.instance.delete_user(user)
        self.refresh_list()
